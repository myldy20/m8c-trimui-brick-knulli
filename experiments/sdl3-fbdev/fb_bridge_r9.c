...
static void log_present_error(const char *message) {
  if (state.present_failures < 5) {
    SDL_LogError(SDL_LOG_CATEGORY_RENDER, "fbdev bridge r9: %s", message);
  }
...
int fb_bridge_init(const char *device_path) {
  ...
  if (state.fd < 0) {
    SDL_LogCritical(SDL_LOG_CATEGORY_VIDEO, "fbdev bridge r9: cannot open %s: %s", device_path,
                    strerror(errno));
    return 0;
  }

  if (ioctl(state.fd, FBIOGET_FSCREENINFO, &state.fixed) < 0) {
    SDL_LogCritical(SDL_LOG_CATEGORY_VIDEO, "fbdev bridge r9: FBIOGET_FSCREENINFO failed: %s",
                    strerror(errno));
    fb_bridge_close();
    return 0;
  }
  ...
  if (ioctl(state.fd, FBIOGET_VSCREENINFO, &state.variable) < 0) {
    SDL_LogCritical(SDL_LOG_CATEGORY_VIDEO, "fbdev bridge r9: FBIOGET_VSCREENINFO failed: %s",
                    strerror(errno));
    fb_bridge_close();
    return 0;
  }
  ...
  if (!framebuffer_is_argb8888()) {
    SDL_LogCritical(SDL_LOG_CATEGORY_VIDEO,
                    "fbdev bridge r9: expected Brick ARGB8888 framebuffer, got bpp=%u",
  ...
  if (state.memory == MAP_FAILED) {
    state.memory = NULL;
    SDL_LogCritical(SDL_LOG_CATEGORY_VIDEO, "fbdev bridge r9: mmap failed: %s", strerror(errno));
    fb_bridge_close();
    return 0;
  }
  ...
  SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO,
              "fbdev bridge r9: %s %ux%u virtual=%ux%u offset=%u,%u bpp=%u stride=%u "
  ...
  SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO,
              "fbdev bridge r9: completed ARGB8888 SDL surface -> cached row expansion; no "
...
bool fb_bridge_present(SDL_Surface *surface) {
  ...
  if (state.last_source_width != surface->w || state.last_source_height != surface->h) {
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO,
                "fbdev bridge r9: source=%dx%d scaled=%dx%d destination=%dx%d offset=%d,%d",
