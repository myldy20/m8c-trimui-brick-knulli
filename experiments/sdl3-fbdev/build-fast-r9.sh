...
BUILD_DIR="${BUILD_DIR:-/tmp/m8c-fbdev-fast-r9-build}"
...
apt-get install -y --no-install-recommends \
    ca-certificates curl git build-essential binutils pkg-config python3 \
    libserialport-dev patchelf zip
...
cd "$BUILD_DIR"
curl -fL "https://github.com/libsdl-org/SDL/releases/download/release-${SDL_VERSION}/SDL3-${SDL_VERSION}.tar.gz" -o SDL3.tar.gz
tar -xzf SDL3.tar.gz
...
cp /work/experiments/sdl3-fbdev/audio_sdl_brick_thread.c m8c/src/backends/audio_sdl.c
...
python3 /work/experiments/sdl3-fbdev/patch-r5.py m8c
python3 /work/experiments/sdl3-fbdev/patch-r7-raw-controls.py m8c
python3 /work/experiments/sdl3-fbdev/patch-r8-controls-and-rate.py m8c
cp /work/experiments/sdl3-fbdev/fb_bridge_r9.c m8c/src/fb_bridge.c
cp /work/experiments/sdl3-fbdev/fb_bridge_r9.h m8c/src/fb_bridge.h
python3 /work/experiments/sdl3-fbdev/patch-r9-surface-and-audio.py m8c

cd m8c
make CFLAGS="-DBRICK_GAMEPAD_ONLY -DBRICK_RAW_CONTROLS" -j"$(nproc)"
cd "$BUILD_DIR"
...
framebuffer=/dev/fb0
framebuffer_mode=completed-surface-row-expand
render_resolution=320x240
render_readback=false
callback_rate_hz=120
input_mode=raw-joystick
controller_guid=03006aae5e0400008e02000014010000
controls=A-play,B-shift,X-edit,Y-option
select_action=exit-modifier-only
start_action=unused
exit_combination=select-plus-b
ignored_controls=Start,L1,R1,L2,R2,volume-minus,volume-plus
raw_buttons=B0,A1,X2,Y3,L1-4,R1-5,L2-6,R2-7,Select-8,Start-9
raw_dpad=hat0
...
grep -a -q 'Brick Select+B exit combination received' "$APP_DIR/m8c-bin"
grep -a -q 'fbdev bridge r9' "$APP_DIR/m8c-bin"
grep -a -q 'no render readback' "$APP_DIR/m8c-bin"
